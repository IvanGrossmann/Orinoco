    ...
    if ( panier == null) {
        ...
        Object.values(panier).map( (furniture) => {
            ...
            afficheProduitPanier.appendChild(tr)
                ...
                var vernis = document.createElement("td")
                vernis.textContent = furniture.varnish
                tr.appendChild(vernis)
...
//console.log(prenom, nom, email, adresse, ville)
// prenom
prenom.addEventListener('input', ({ target: { value } }) => {   
})
// nom
...
// adresse
adresse.addEventListener('input', ({ target: { value } }) => {
})
// ville
ville.addEventListener('input', ({ target: { value } }) => {
})
